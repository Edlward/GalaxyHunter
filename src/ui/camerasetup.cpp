...
void CameraSetup::setCamera(const ImagerPtr& imager, const ImagingSequence::ptr &imagingSequence)
{
    d->ui->imageSettings->disconnect();
    d->ui->imageSettings->setDisabled(true);
    d->imager = imager;
    if(imagingSequence) {
        qDebug() << imagingSequence;
        d->imagerSettings = imagingSequence->imagerSettings();
